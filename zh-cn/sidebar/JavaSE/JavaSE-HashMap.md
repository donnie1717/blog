    ...
    final Node<K,V>[] resize() {
        ...
        if (oldTab != null) {
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null;
                    if (e.next == null) // 无链表或红黑树，则重新Hash确定位置
                        ...
                        do {
                            next = e.next;
                            //判断key的hash值与老数组长度与操作后结果决定元素是放在原索引处还是新索引，如key的值为1、17，oldCap为16 
                            if ((e.hash & oldCap) == 0) {
                                if (loTail == null)
                                    loHead = e;
                                else
...
### 总结
数组大小n总是2的整数次幂，因此计算下标时直接( hash & n-1)，这样的好处就是可以直接取代取模运算，提高计算速度。分配内存初始化通过resize()方法 ，主要时初始化的时候和put方法超过阈值的时候扩容，因此最好是在构造函数就进行初始化。哈希冲突时转为链表存储，如果链表的长度大于8则转化为红黑树。
...
```
位移：主要是为了减少碰撞，降低Hash冲突的概率。右移16位加异或刚好能保留高16位和低16位的特征。HashMap利用**(n - 1) & hash**来定位key的位置，n为数组的长度。当数组的长度很小的时候比如16，如果没有通过右移16位和异或，只通过数组的长度取余，Hash冲突的概率会加大。
异或：使用&会使结果趋于1，使用|会使结果趋于0，使用^能够使高16位和低16位的信息尽量保存。